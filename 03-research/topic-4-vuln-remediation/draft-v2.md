...
revision_notes: |
  Round 1 review (review-round-1.md) flagged 8 blocking issues. This revision:
  (1) Replaces Qualys + GreyNoise (1500-word floor failures) with VulnCheck State of
      Exploitation 2026 + TechTarget DBIR-2026 takeaways. Qualys and GreyNoise data
      are now cited as DBIR-referenced primary data via the DBIR footnotes, not as
      independent third-party sources.
  (2) GreyNoise 11-day lead-time is now properly qualified (68/104 surges = 65%
      preceded a CVE; the median is over that subset only). Counter-evidence from
      VulnCheck and DBIR-2025 edge data is integrated in §3.
  (3) First-appearance definitions added for KEV, EPSS, SSVC, ASM, MTTR, virtual
      patching, n-day, IAB, MOVEit, CISA.
  (4) §5 control #5 split into 5a/5b/5c with concrete acceptance criteria.
  (5) 31% framing qualified at first use in §2.
  (6) §5 renumbered into Telemetry T1-T4 / Signatures S1-S3 / Controls C1-C7
      (Suggestion #1, accepted).
  (7) §2 keyword-density framing moved to footnote; breach-share trajectory
      (<6% → 20% → 31%) is now the lede (Suggestion #2).
  (8) Cross-taxonomy-break guardrail sentence added to §2 (Suggestion #8).
  (9) §6 cohort-comparability caveat added (Suggestion #5).
  (10) §5 C2 "CVSS+EPSS+KEV defensible ranking" flagged as author recommendation;
       FIRST.org EPSS user guide cited (Suggestion #4).
  (11) §7 #5 "Rust/Go" softened to "memory-safe languages and frameworks"
       (Suggestion #6).
  (12) §3 T1588.005 IAB citation rebased to DBIR-2023 p.35 with author-mapping
       footnote (Suggestion #7).
  (13) §4 MOVEit cite expanded to [DBIR-2024, p.34-35] (Suggestion #3).
---
